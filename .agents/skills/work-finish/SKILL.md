...
`pending_memory_candidates`가 있으면 최종 보고에 남은 후보 수를 포함한다. MemoryGraph Finalize Skill이 구현되기 전에는 후보를 임의로 promote 또는 reject하지 않으며, 후보가 있다는 이유만으로 Run 종료를 막지 않는다.
